...
#include <jni.h>
#include <audio/AudioRenderWrapper.h>
#include <video/VideoRenderWrapper.h>
...
class Output
{
    public:
        ...
        static int AudioDriver_write(void * buffer,
                                     int    buffer_size);

        static int AudioDriver_unregister();

        static int VideoDriver_register(JNIEnv * env,
                                        jobject  jsurface);

        static int VideoDriver_getPixels(int     width,
                                         int     height,
                                         void ** pixels);

        static int VideoDriver_updateSurface(bool autoScale);
